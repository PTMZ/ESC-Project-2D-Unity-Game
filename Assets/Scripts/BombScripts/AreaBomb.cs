...
public class AreaBomb : MonoBehaviour
{
    ...
    //CircleCollider2D explosionGrassRadius;

    void Start()
    {
        explosionRadius = gameObject.GetComponent<BoxCollider2D>() as BoxCollider2D;
        //explosionGrassRadius = gameObject.GetComponent<CircleCollider2D>() as CircleCollider2D;
    ...
    */
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        if (collision.gameObject.GetComponent<PlayerAvatar>() != null && exploded == true)
        {
            //Debug.Log("I am hit, my name is " + collision.gameObject.name);
            collision.gameObject.GetComponent<PlayerAvatar>().getHit(bombDamage);
            Vector2 target = collision.gameObject.GetComponent<PlayerAvatar>().transform.position;
        ...
        /*
        if (collision.CompareTag("Grass") && exploded == true)
        {
            collision.gameObject.GetComponent<MinesweeperGrass>().getBombed();
        }
    ...
    public void changeBombState(bool newState)
    {
        bombState = newState;
    }
